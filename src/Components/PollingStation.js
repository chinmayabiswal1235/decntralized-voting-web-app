...
import LoadingCircle from '../assets/loadingcircles.svg';

const PollingStation = (props) => {
    const [candidate1URL, changeCandidate1Url] = useState(LoadingCircle);
    const [candidate2URL, changeCandidate2Url] = useState(LoadingCircle);
        ...
        const getInfo = async () => {
          ...
          changeCandidate1Url(
            await window.contract.getUrl({
              name: localStorage.getItem("Candidate1") && JSON.parse(localStorage.getItem("Candidate1"))?.name,
            })
          );
          changeCandidate2Url(
            await window.contract.getUrl({
             name: localStorage.getItem("Candidate2") && JSON.parse(localStorage.getItem("Candidate2"))?.name,
            })
          );
